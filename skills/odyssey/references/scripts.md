...
- `scripts/record-verify.mjs <repo> <slug> <todo-id> --criterion <cmd> --exit-code <N> [--output <file>] [--n <idx>]` — records per-criterion evidence under `.zcode/verify/`. Exit 6 on failure (exit-code != 0). NOTE (security): the exit-code is a caller-supplied argv, not executed — see external-audit finding; the orchestrator must run the criterion itself and pass the real exit code.
...
- `scripts/status.mjs <repo> <slug> [--json]` — quick "where is this run?" (phase, verdict, todo counts).
