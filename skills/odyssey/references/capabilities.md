...
- **Research-kind runs (rows 33-34, hyperresearch adaptations)** — when the deliverable is a report/study/survey, the plan carries a `## Deliverable contract` (register/format/tier levers + ordered literal headings + atomic items incl. period-pinned figures — `parse-plan --lint` refuses a vacuous shape, momus rejects its absence); research dispatches carry the canonical question VERBATIM block-quoted; librarian findings cite verbatim quoted passages + permalinks with `contested:`/syndication markers; explore answers cite `path:line` spans; synthesis emits a Tensions section; research notepads end with a one-line drift check (`Answers the question: yes|partial|no — why`).
