...
> the routing table. Every agent (metis, prometheus, momus, sisyphus-junior, and the conductor itself)
> consults it before choosing how to work. When a capability here fits your current activity, USE IT —
...
| **Hard multi-step reasoning** | `sequentialthinking` MCP | `Task: oracle` (ours) |
| **Plan the work** | `Task: prometheus` (ours) | `skill: writing-plans`, `skill: executing-plans` |
| **Research the codebase** | `codegraph_explore` MCP (if `.codegraph/`) | `Task: explore` (ours), `Task: librarian` (docs/OSS) |
| **Research libraries/docs** | `Context7` MCP + `Task: librarian` | `WebSearch` / `WebFetch` |
| **Design / architecture** | `Task: oracle` (ours) + `skill: brainstorming` | `feature-dev:code-architect`, `skill: premortem`, `sequentialthinking` MCP |
| **UI/UX design intelligence** | `skill: ui-ux-pro-max` | `Task: oracle` (design review), `playwright`/`chrome-devtools` MCPs (UI verify) |
| **Implement (TDD)** | `skill: test-driven-development` | `skill: subagent-driven-development`, `Task: sisyphus-junior` |
| **Implement (plan execution)** | `skill: executing-plans` | `skill: using-git-worktrees` (isolation) |
| **Debug (hard)** | `skill: systematic-debugging` + `sequentialthinking` MCP | `Task: oracle` (after 2 failed attempts) |
...
| **Review a plan (gate)** | `Task: momus` (ours) | `Task: oracle` (independent, for architecture) |
...
| **Media / image / PDF** | `Task: multimodal-looker` (ours) | — |
...
- **`skill: premortem`** — for `architecture` and `mid-sized` intents, Metis should run a premortem to surface failure modes, then fold them into her directives.
- **`codegraph_explore`** — if the repo is indexed, Metis should probe it (one call) to ground her risk analysis in real structure, not assumptions. Fallback: `Task: explore`.
...
- **`skill: writing-plans`** (superpowers) — Prometheus MUST consult this alongside the odyssey scaffold; it's the battle-tested plan-writing method and complements our plan contract.
...
- **`Task: explore` / `Task: librarian` / `codegraph_explore`** — fan out for research BEFORE drafting (Metis will have recommended which).
- **`skill: using-git-worktrees`** — if the work would touch a busy repo, the plan should propose a worktree for isolation.
- **`skill: ui-ux-pro-max`** (UI/UX tasks only) — the ORCHESTRATOR runs the design-database search (not prometheus — sub-agents can't load skills) BEFORE prometheus drafts, and passes the results into prometheus's dispatch prompt. Search order: product type → style → typography → color → landing (or chart for dashboards) → stack. The design context grounds the plan's visual decisions so prometheus doesn't invent a design system from scratch. Key search: `python3 ~/.zcode/skills/ui-ux-pro-max/scripts/search.py "<keyword>" --domain <domain> -n 3` for each relevant domain.

### Phase 3 — Review (`momus`, the gate)
- **`Task: momus`** (ours) — the primary gate; checks executability, references, QA.
- **`Task: oracle`** (ours) — for `architecture` intent, an INDEPENDENT second review (both must OKAY). Oracle catches what Momus (a blocker-finder) won't: design-level flaws.
- **`skill: premortem`** — on REJECT, before Prometheus revises, a premortem on the rejected plan often surfaces why.

### Phase 4 — Execute (`sisyphus-junior`)
...
- **`skill: subagent-driven-development`** — when a todo is large, the executor splits it into sub-tasks delegated to further sisyphus-junior instances (parallel where independent).
...
- **`scripts/judge.mjs`** — INDEPENDENT LLM-as-judge on the external CLI (not oracle — oracle is a participant); scores the final diff against the seed's success_criteria on the MEASUREMENT.md rubric; appends to `eval/judged.jsonl`.
