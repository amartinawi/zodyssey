...
- **Ground the premortem in prior-run signals** — recommend the orchestrator run BOTH `recall-outcomes.mjs <repo>` (prior blocked/failed OUTCOMES for this repo) AND `recall-corrections.mjs <repo>` (correction SIGNALS: verify-fail re-dispatches, Momus REJECT+blockers, top-K bounded) at consult. Sub-agents can't spawn Bash, so the orchestrator runs the scripts and passes the output, same model as the codegraph/MCP bullets above. Fold any surfaced verify-fail / REJECT-blocker signals into your premortem's Identified Risks.
- **Weigh narrator reliability before trusting verdict history** — also run `registry-report.mjs <repo>` (the narrator trust registry: cross-run agent-config reliability mined from external-audit verdicts and judge criterion results; trust is Laplace-smoothed and ALWAYS carries its sample count n — a decimal without n is not evidence). Orchestrator runs it and passes the output, same model as the recall bullets. Fold low-trust/high-n entries into your premortem's Identified Risks: a momus config that missed several external audits justifies recommending the independent oracle co-review at phase 3 and stricter QA/acceptance-criteria directives; a low-trust executor config justifies smaller todo granularity. Small-n (n < 3) is cold-start noise — note it, don't act on it.
