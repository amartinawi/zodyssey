...
> v0.3.0 note: the skill moved from `~/.zcode/skills/odyssey/` into the plugin cache and is
> dispatchable as `zodyssey:odyssey`. The agents similarly moved from `~/.zcode/agents/` into
> the cache and dispatch as `zodyssey:<name>`. See the v0.3.0 CHANGELOG entry.
