...
11. set-phase.mjs:99-105 — `done --force` bypasses the final.verdict precondition; done is TERMINAL → disarms all hooks. The DAG check (:93) does NOT save it (final→done is a valid transition). Verify --force can no longer target done.
...
17. CLAUDE_CLI (consult.mjs:456, judge.mjs:262) is taken raw, unvalidated, despite the "Environment overrides" section of SKILL.md (SKILL.md:427) formerly claiming "all validated". Verify it's validated or the SKILL.md claim is corrected.
