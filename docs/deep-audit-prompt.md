...
13. post-tool.mjs:88-107 + stop.mjs:40-64 — findActiveRun is FLAT (no DFS); known-issue #6a was fixed in pre-tool.mjs only. Nested-repo runs: parallel-cap ledger never drains (30-min stall), resume checkpoint never written. Verify findActiveRun was extracted to a shared module.
...
17. CLAUDE_CLI (consult.mjs:339, judge.mjs:110) is taken raw, unvalidated, despite the "Environment overrides" section of SKILL.md (SKILL.md:360) formerly claiming "all validated". Verify it's validated or the SKILL.md claim is corrected.
