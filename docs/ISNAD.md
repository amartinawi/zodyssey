...
**A numbering collision to watch.** `R2` and `R3` already denote something unrelated in this repo — bash command-classification remediation rounds ([`docs/OPPORTUNITY-MAP.md:333`](OPPORTUNITY-MAP.md), `CHANGELOG.md:551-552`). Two rule namespaces share the same tokens; read the surrounding context, not the number.
