...
- **Evidence:** `skills/odyssey/scripts/compact.mjs` header — "OPT-IN … not wired into any phase
  ...
  non-empty lines); `skills/odyssey/SKILL.md:462` (memory-MCP knowledge graph written "at end of
...
2. `skills/odyssey/SKILL.md:462` — the canonical cross-run store (memory-MCP knowledge graph)
...
- **The repo's own history is a natural experiment on the bet.** Every mechanism left to prompt
  ...
  nothing (`skills/odyssey/SKILL.md:462`); token telemetry populated 1 of 177 records; `compact.mjs`
...
6. **Context/memory weakest** (map §1.1 ≡ §2): both argue it from `compact.mjs`'s unwired
   truncation, the convention-only memory store (`SKILL.md:462`), and the recency-list retrieval
