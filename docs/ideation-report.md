...
- **Evidence:** `skills/odyssey/references/scripts.md:47` — "Run it during verify on the run's
  ...
  `skills/odyssey/scripts/check-imports.mjs:1-23` + `CHANGELOG.md:851` (shipped v0.3.2, offline
...
- **Evidence:** `skills/odyssey/scripts/judge.mjs:311` (`arm: "zodyssey"` literal in the record
  ...
  `CHANGELOG.md:272` (prints instructions instead of running — pre-09 state); `~/.zcode/orchestration/eval/judged.jsonl` — 5 scored
...
- **Evidence:** `CHANGELOG.md:464` (named terminus, deliberately unshipped — "it wants its own
...
(explicitly blocked on harness support, `CHANGELOG.md:493`); any additional reviewer/verifier agent
...
instructions instead of running (`harness.mjs:21-23`, pre-09 state narrated at `CHANGELOG.md:272`); `judged.jsonl` holds 5 real scored
