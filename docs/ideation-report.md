...
- **Evidence:** `skills/odyssey/references/scripts.md:47` — "Run it during verify on the run's
  ...
  `skills/odyssey/scripts/check-imports.mjs:1-23` + `CHANGELOG.md:984` (shipped v0.3.2, offline
  import resolution, exit 9 on unresolved); the wiring pattern is proven in-repo by B8:
  `skills/odyssey/scripts/set-phase.mjs:363` auto-invokes the regression gate at the execute
...
- **Evidence:** `skills/odyssey/scripts/lib/tokens.mjs:36` (reads `~/.zcode/cli/db/db.sqlite`),
  ...
  run-report.mjs:151-160` wires it in; results.jsonl inspected directly: 177 records, 1 populated
...
- **Evidence:** `skills/odyssey/scripts/compact.mjs` header — "OPT-IN … not wired into any phase
  ...
  non-empty lines); `skills/odyssey/SKILL.md:463` (memory-MCP knowledge graph written "at end of
...
- **Evidence:** `skills/odyssey/scripts/judge.mjs:311` (`arm: "zodyssey"` literal in the record
  ...
  (baseline marked TODO), `skills/odyssey/scripts/harness.mjs:90-91` (already parses `--arm zodyssey|baseline`),
  `CHANGELOG.md:373` (prints instructions instead of running — pre-09 state); `~/.zcode/orchestration/eval/judged.jsonl` — 5 scored
...
- **Evidence:** `CHANGELOG.md:577` (named terminus, deliberately unshipped — "it wants its own
...
- **Evidence:** `~/.zcode/cli/plugins/cache/zodyssey-local/zodyssey/` — 6 dirs on disk (0.3.2,
  ...
  contains no pruning, only a stale-cache warning at `install.mjs:977` (notepad 1, row c-cache).
...
(explicitly blocked on harness support, `CHANGELOG.md:658`); any additional reviewer/verifier agent
...
| Guardrails | **STRONG** | review gate (`pre-tool.mjs:894`), fail-closed scope isolation (`:940`), hook-minted nonces (`:1456-1485`), plan-tamper guard (`:916,1287`), HMAC run discovery (`:30,516`) — the project's center of mass |
...
2. `skills/odyssey/SKILL.md:463` — the canonical cross-run store (memory-MCP knowledge graph)
...
3. `skills/odyssey/scripts/recall-corrections.mjs:2-25` + `recall-outcomes.mjs:2-9` — the entire
   ...
   in-run handoff is "downstream todos read prior notepads by path" (`skills/odyssey/SKILL.md:242`)
...
transition — the B8 regression gate, auto-invoked at `skills/odyssey/scripts/set-phase.mjs:363`
...
`judge.mjs:311` hardcodes the arm; `harness.mjs:21-23` marks baseline TODO while `:90-91` already
...
- **Where the code actually is:** guardrails STRONG (review gate `skills/odyssey/hooks/pre-tool.mjs:894`,
...
- **The repo's own history is a natural experiment on the bet.** Every mechanism left to prompt
  ...
  nothing (`skills/odyssey/SKILL.md:463`); token telemetry populated 1 of 177 records; `compact.mjs`
...
`--arm zodyssey|baseline` (`harness.mjs:90-91`) but the baseline arm is TODO and prints
instructions instead of running (`harness.mjs:21-23`, pre-09 state narrated at `CHANGELOG.md:373`); `judged.jsonl` holds 5 real scored
...
|---|---|---|---|
| (a1) Phase DAG with hook-enforced gates | CONFIRMED | skills/odyssey/SKILL.md:76; skills/odyssey/scripts/set-phase.mjs:90-93 | 8-phase state machine; TRANSITIONS map refuses illegal transitions (set-phase.mjs:286-289); hook gates at pre-tool.mjs:864,1345 |
...
| (a4) Bash/Edit write gate with declared-file scope | CONFIRMED | skills/odyssey/hooks/pre-tool.mjs:1345,1225-1226 | write-capable Bash requires OKAY + declared Files:; Edit path same at :864 (boundary behavior of the Edit path — see Reconciliation M1) |
...
| (a6) Test-integrity guard | CONFIRMED | skills/odyssey/scripts/record-final-wave.mjs:138,253-256 | SKIP_MARKER regex + `git diff --numstat` flags deleted/net-weakened test files |
| (a7) Pass-to-pass regression gate | CONFIRMED | skills/odyssey/scripts/regression-gate.mjs:1; skills/odyssey/scripts/set-phase.mjs:363 | auto-snapshot entering execute; exit 8 on pass→fail; done blocked while regressed |
...
| (a10) Real token accounting from the session DB | CONFIRMED — at skills/odyssey/scripts/lib/tokens.mjs, reading ~/.zcode/cli/db/db.sqlite | skills/odyssey/scripts/lib/tokens.mjs:36,83,105-114; skills/odyssey/scripts/run-report.mjs:18,157 | node:sqlite read-only; SQL over model_usage JOIN session by repo+time-window; attribution honestly "estimate" |
...
| (a12) "~98 gate-surface cases" | CONFIRMED (re-measured; exactly 98 — not approximate) | skills/odyssey/hooks/pre-tool.gate-surface.test.mjs:1; CHANGELOG.md:581 | standalone run: "98 passed, 0 failed"; CHANGELOG v0.5.2: "Gate-surface is 98 cases (was 80)" |
| (a13) Paired old/new probes | CONFIRMED | skills/odyssey/hooks/pre-tool.gate-surface.test.mjs:6; CHANGELOG.md:515,534; skills/odyssey/hooks/pre-tool.bash-gate.test.mjs:17 | "Every case below fails on the pre-v0.5.0 code"; v0.5.2 paired run against v0.5.1 |
...
| (b-B9-1) Package-existence checking "absent" | REFUTED — code wins | skills/odyssey/scripts/check-imports.mjs:1-23; CHANGELOG.md:984 | check-imports.mjs exists (shipped v0.3.2): offline import resolution JS/TS + Python; exit 9 on unresolved import |
...
| (b-judge-2) judge.mjs "never reads --arm" | CONFIRMED for judge.mjs (nuance: harness parses it) | skills/odyssey/scripts/judge.mjs:150; skills/odyssey/scripts/harness.mjs:90-91, skills/odyssey/scripts/harness.mjs:19, skills/odyssey/scripts/harness.mjs:149-152 | judge parses only `--double`; harness parses `--arm` but baseline is TODO and prints instructions (state at measurement, 2026-08-16; the baseline arm EXECUTES since v0.6.7 — `harness.mjs:149-152` then described the pre-09 instructions block) |
...
| (b-B5) Test files read-only at hook layer | CONFIRMED shipped | skills/odyssey/hooks/pre-tool.mjs:935,1473 | Edit path "test files are read-only during phase=…"; Bash path "test files are FROZEN" |
...
| (b-B9-3) Phase B item B9 shipped | CONFIRMED shipped as standalone script, NOT phase-wired | CHANGELOG.md:984; skills/odyssey/references/scripts.md:47 | shipped v0.3.2; pipeline presence is prompt-convention only |
...
| (c-F) Shell-escaping splits command tokens | CONFIRMED | CHANGELOG.md:561; skills/odyssey/hooks/pre-tool.mjs:184,249-272 | `p\ython -c`, `py''thon -c` defeat regex tokenization; no shell-grammar parser |
| (c-G) Interpreter deny-list unbounded by construction | CONFIRMED (posture inverted; names still enumerated) | CHANGELOG.md:606; skills/odyssey/hooks/pre-tool.mjs:126,140-154 | gawk/mawk/pypy/perl6/raku/jshell/ts-node ungated; posture inverted to allowlist-of-gated-names, but the NAME list remains unbounded |
| (c-H) Accepted over-blocks | CONFIRMED | CHANGELOG.md:563; skills/odyssey/hooks/pre-tool.mjs:173-192 | `/usr/bin/git status` gated (path-heads classified as execution); over-block asserted deliberately in the suite |
| (c-head-allowlist) Head-allowlist inversion unshipped | CONFIRMED | CHANGELOG.md:577; skills/odyssey/hooks/pre-tool.mjs:100-199 | "deliberately **not** in this release: it wants its own release and its own paired run"; code remains a deny-list |
| (c-nonces) Nonces prove dispatched-not-said | CONFIRMED (still true; fix NOT done) | skills/odyssey/scripts/record-final-artifact.mjs:126-132; CHANGELOG.md:658; skills/odyssey/hooks/pre-tool.mjs:1716 | nonce lives in agent-readable .zcode/state/; transcript-hash binding "NOT done" (needs harness support) |
| (c-cache) Five stale cache versions, no pruning | CONFIRMED (5 stale + 1 live) | ~/.zcode/cli/plugins/cache/zodyssey-local/zodyssey/ (6 dirs); package.json:3 (0.5.2 at measurement; 0.6.x across the 2026-08-17/18 releases, cache follows on re-Get); scripts/install.mjs:977 | 0.3.2–0.5.1 stale, 0.5.2 live; install.mjs only warns, never prunes |
...
6. **Context/memory weakest** (map §1.1 ≡ §2): both argue it from `compact.mjs`'s unwired
   truncation, the convention-only memory store (`SKILL.md:463`), and the recency-list retrieval
...
10. **Cache pruning residual** (map item #13 ≡ §1 entry 8): 5 stale versions, `install.mjs:977`
...
(`CHANGELOG.md:577`); the map demotes it ("flips the sign but stays in the same game," §2.1).
...
remained the unbounded residual (`:152`, `CHANGELOG.md:606`) — a sign-flip alone demonstrably did
...
exit(0);` — one environment variable disables the entire Bash gate, and `scripts/install.mjs:975`
