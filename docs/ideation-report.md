...
- **Evidence:** `skills/odyssey/references/scripts.md:47` — "Run it during verify on the run's
  ...
  `skills/odyssey/scripts/check-imports.mjs:1-23` + `CHANGELOG.md:944` (shipped v0.3.2, offline
...
- **Evidence:** `skills/odyssey/scripts/compact.mjs` header — "OPT-IN … not wired into any phase
  ...
  non-empty lines); `skills/odyssey/SKILL.md:439` (memory-MCP knowledge graph written "at end of
...
- **Evidence:** `skills/odyssey/scripts/judge.mjs:311` (`arm: "zodyssey"` literal in the record
  ...
  `CHANGELOG.md:333` (prints instructions instead of running — pre-09 state); `~/.zcode/orchestration/eval/judged.jsonl` — 5 scored
...
- **Evidence:** `CHANGELOG.md:525` (named terminus, deliberately unshipped — "it wants its own
...
(explicitly blocked on harness support, `CHANGELOG.md:618`); any additional reviewer/verifier agent
...
2. `skills/odyssey/SKILL.md:439` — the canonical cross-run store (memory-MCP knowledge graph)
...
3. `skills/odyssey/scripts/recall-corrections.mjs:2-25` + `recall-outcomes.mjs:2-9` — the entire
   ...
   in-run handoff is "downstream todos read prior notepads by path" (`skills/odyssey/SKILL.md:225`)
...
- **The repo's own history is a natural experiment on the bet.** Every mechanism left to prompt
  ...
  nothing (`skills/odyssey/SKILL.md:439`); token telemetry populated 1 of 177 records; `compact.mjs`
...
instructions instead of running (`harness.mjs:21-23`, pre-09 state narrated at `CHANGELOG.md:333`); `judged.jsonl` holds 5 real scored
...
|---|---|---|---|
| (a1) Phase DAG with hook-enforced gates | CONFIRMED | skills/odyssey/SKILL.md:65; skills/odyssey/scripts/set-phase.mjs:90-93 | 8-phase state machine; TRANSITIONS map refuses illegal transitions (set-phase.mjs:286-289); hook gates at pre-tool.mjs:864,1345 |
...
| (a12) "~98 gate-surface cases" | CONFIRMED (re-measured; exactly 98 — not approximate) | skills/odyssey/hooks/pre-tool.gate-surface.test.mjs:1; CHANGELOG.md:529 | standalone run: "98 passed, 0 failed"; CHANGELOG v0.5.2: "Gate-surface is 98 cases (was 80)" |
| (a13) Paired old/new probes | CONFIRMED | skills/odyssey/hooks/pre-tool.gate-surface.test.mjs:6; CHANGELOG.md:475,494; skills/odyssey/hooks/pre-tool.bash-gate.test.mjs:17 | "Every case below fails on the pre-v0.5.0 code"; v0.5.2 paired run against v0.5.1 |
...
| (b-B9-1) Package-existence checking "absent" | REFUTED — code wins | skills/odyssey/scripts/check-imports.mjs:1-23; CHANGELOG.md:944 | check-imports.mjs exists (shipped v0.3.2): offline import resolution JS/TS + Python; exit 9 on unresolved import |
...
| (b-B9-3) Phase B item B9 shipped | CONFIRMED shipped as standalone script, NOT phase-wired | CHANGELOG.md:944; skills/odyssey/references/scripts.md:47 | shipped v0.3.2; pipeline presence is prompt-convention only |
...
| (c-F) Shell-escaping splits command tokens | CONFIRMED | CHANGELOG.md:521; skills/odyssey/hooks/pre-tool.mjs:184,249-272 | `p\ython -c`, `py''thon -c` defeat regex tokenization; no shell-grammar parser |
| (c-G) Interpreter deny-list unbounded by construction | CONFIRMED (posture inverted; names still enumerated) | CHANGELOG.md:566; skills/odyssey/hooks/pre-tool.mjs:126,140-154 | gawk/mawk/pypy/perl6/raku/jshell/ts-node ungated; posture inverted to allowlist-of-gated-names, but the NAME list remains unbounded |
| (c-H) Accepted over-blocks | CONFIRMED | CHANGELOG.md:523; skills/odyssey/hooks/pre-tool.mjs:173-192 | `/usr/bin/git status` gated (path-heads classified as execution); over-block asserted deliberately in the suite |
| (c-head-allowlist) Head-allowlist inversion unshipped | CONFIRMED | CHANGELOG.md:525; skills/odyssey/hooks/pre-tool.mjs:100-199 | "deliberately **not** in this release: it wants its own release and its own paired run"; code remains a deny-list |
| (c-nonces) Nonces prove dispatched-not-said | CONFIRMED (still true; fix NOT done) | skills/odyssey/scripts/record-final-artifact.mjs:126-132; CHANGELOG.md:618; skills/odyssey/hooks/pre-tool.mjs:1716 | nonce lives in agent-readable .zcode/state/; transcript-hash binding "NOT done" (needs harness support) |
...
6. **Context/memory weakest** (map §1.1 ≡ §2): both argue it from `compact.mjs`'s unwired
   truncation, the convention-only memory store (`SKILL.md:439`), and the recency-list retrieval
...
(`CHANGELOG.md:525`); the map demotes it ("flips the sign but stays in the same game," §2.1).
...
remained the unbounded residual (`:152`, `CHANGELOG.md:566`) — a sign-flip alone demonstrably did
