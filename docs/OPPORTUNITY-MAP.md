...
The real blocker is `harness.mjs:11-14` and `:128-136`: the baseline arm prints instructions to a
...
`set-phase.mjs:245-253` writes a fixed two-line template on every terminal transition. The entire
...
`set-phase.mjs:217-226` appends every terminal run's scorecard to
...
| **11** | **B10 pre-edit lint baseline** | Pre-existing lint noise is attributed to the edit | **No published outcome evidence.** Internal reasoning is sound | ~35 lines | Slower post-edit hook |
