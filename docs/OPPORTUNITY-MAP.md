...
the pre-item-09 state, narrated at `CHANGELOG.md:313`; the zodyssey arm is not automated either.
...
| B8 pass-to-pass regression gate | **HALF-SHIPPED — corrected 2026-08-16.** The `--snapshot` half is wired (`set-phase.mjs:363`, `record-review.mjs:295`). The `--check` half has **zero code callers**, and `--check` is the only writer of `status: "regressed"` (`regression-gate.mjs:181`) — the field `set-phase.mjs:132` blocks `done` on. The baseline is taken automatically; the comparison never runs, so the gate has never fired in an automated path |
...
`set-phase.mjs:517-525` writes a fixed two-line template on every terminal transition. The entire
...
`set-phase.mjs:478-498` appends every terminal run's scorecard to
...
| **8** | **Emit OTel GenAI spans** (`invoke_agent` → `execute_tool`) | Replaces time-window token attribution with real per-dispatch attribution, and makes the data legible to standard tooling | Industry converged; Claude Code emits (beta). **Caveat: every `gen_ai.*` attribute is still "Development" stability, none Stable (re-checked 2026-08-23: unchanged; `gen_ai.*` was split into a dedicated `semantic-conventions-genai` repo at core v1.42.0 / 2026-06-12, which still has no tagged release)** | Medium | Nothing; additive |
...
stability; none is Stable. Re-checked 2026-08-23: unchanged — the `gen_ai.*` conventions moved to a dedicated `semantic-conventions-genai` repo (core v1.42.0, 2026-06-12) with no tagged release yet.**
