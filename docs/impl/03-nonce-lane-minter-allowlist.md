...
one-security-change-per-release cadence, `CHANGELOG.md:505` — grouped adjacently with 01/04 because
...
(`skills/odyssey/SKILL.md:410`: "the nonce only exists after a real `Task(zodyssey:momus)` call the
...
- `skills/odyssey/hooks/pre-tool.mjs:1344` — `const isAgent = (want) => sameName(want, subagent);`
...
`skills/odyssey/hooks/pre-tool.mjs:1583` (was `isAgent("code-reviewer")` →
`mintNonceFor("final_f2")` at `:1570`) and `:1574` (was `isAgent("oracle")` →
`mintNonceFor("final_f4")` at `:1580`). An `evil:code-reviewer` or `evil:oracle` dispatch minted a
...
(`CHANGELOG.md:606` and the comment at `skills/odyssey/hooks/pre-tool.mjs:1510-1517`) —
...
`sameName` via `isAgent` at `skills/odyssey/hooks/pre-tool.mjs:1344` (imported at `:32`). The
...
2. A Task dispatch whose type's final segment is `momus` but which is not the declared minter
   ...
   (`skills/odyssey/hooks/pre-tool.mjs:1371`, `inSet`) deliberately grants no authority — but
...
3. The same exactness holds at the sibling lanes: `final_f2` mints only for the declared
   ...
   nothing.
4. The round-cap + pre-dispatch-lint site at `skills/odyssey/hooks/pre-tool.mjs:1518` agrees with
   the minter on what counts as `momus` (the rule v0.5.1's audit-3 finding 7 stated at
   `:1495-1496`: "the two sites must agree … or the guard is decorative"). A lookalike is neither
...
6. Everything else is byte-identical: the phase-gate tolerance (`:1352`) is untouched; F5
...
- Do not batch this into 01's or 04's release in the CHANGELOG shape — one security change per
  release (`CHANGELOG.md:505`: a structural gate change "wants its own release and its own paired
  run").
...
1. **Enumeration instead of structure.** Not a deny-list of lookalike names: it is an identity
   ...
   `CHANGELOG.md:505` (classify by known-safe identity, not by denied shapes). There is no shape to
...
(`skills/odyssey/SKILL.md:406`, `skills/odyssey/references/scripts.md:59`). The only
...
(the v0.5.1 Class C failure mode, `skills/odyssey/hooks/pre-tool.mjs:1330-1343`) now surfaces
...
the v0.5.2 head-allowlist inversion named at `CHANGELOG.md:505`: identity questions ("who may
...
v0.5.1 audit-3 finding 7 (`CHANGELOG.md:606`): the round-cap twin and the minter disagreed, and
...
`skills/odyssey/hooks/pre-tool.mjs:1330-1343` documents (tolerance was chosen there to stop silent
...
- `README.md:135` — comparison-table row "Review verdicts are read, not assumed": extend the
  ...
  lookalike namespace.
- `skills/odyssey/SKILL.md:406-412` — the review-gate chain description: the sentence "the nonce
...
change per release, shipped alone** — the repo rule with its precedent at `CHANGELOG.md:505`. Do
...
- **Known, not fixed** — name them; the next audit should not have to find them:
  ...
  - The orchestrator-adversary residual stands unchanged (`skills/odyssey/SKILL.md:412`: the nonce
  ...
  - Case variance narrows with the same stroke (found in run verification, pinned by no criterion):
    ...
    dispatches lowercase (the extractor at `skills/odyssey/hooks/pre-tool.mjs:1328` strips
  ...
  - The `agent_type` / `type` fallback fields in the extractor at
    `skills/odyssey/hooks/pre-tool.mjs:1328` are unchanged and out of scope.
...
the declaration against hook-witnessed loads (`skills/odyssey/references/scripts.md:23`), so a
