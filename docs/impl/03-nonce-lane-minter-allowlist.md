...
one-security-change-per-release cadence, `CHANGELOG.md:184` — grouped adjacently with 01/04 because
...
- Do not batch this into 01's or 04's release in the CHANGELOG shape — one security change per
  release (`CHANGELOG.md:184`: a structural gate change "wants its own release and its own paired
  run").
...
1. **Enumeration instead of structure.** Not a deny-list of lookalike names: it is an identity
   ...
   `CHANGELOG.md:184` (classify by known-safe identity, not by denied shapes). There is no shape to
...
the v0.5.2 head-allowlist inversion named at `CHANGELOG.md:184`: identity questions ("who may
...
change per release, shipped alone** — the repo rule with its precedent at `CHANGELOG.md:184`. Do
