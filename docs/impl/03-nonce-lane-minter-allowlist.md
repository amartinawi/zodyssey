...
one-security-change-per-release cadence, `CHANGELOG.md:577` — grouped adjacently with 01/04 because
...
(`skills/odyssey/SKILL.md:447`: "the nonce only exists after a real `Task(zodyssey:momus)` call the
...
(`CHANGELOG.md:666` and the comment at `skills/odyssey/hooks/pre-tool.mjs:1510-1517`) —
...
- Do not batch this into 01's or 04's release in the CHANGELOG shape — one security change per
  release (`CHANGELOG.md:577`: a structural gate change "wants its own release and its own paired
  run").
...
1. **Enumeration instead of structure.** Not a deny-list of lookalike names: it is an identity
   ...
   `CHANGELOG.md:577` (classify by known-safe identity, not by denied shapes). There is no shape to
...
(`skills/odyssey/SKILL.md:443`, `skills/odyssey/references/scripts.md:59`). The only
...
the v0.5.2 head-allowlist inversion named at `CHANGELOG.md:577`: identity questions ("who may
...
v0.5.1 audit-3 finding 7 (`CHANGELOG.md:666`): the round-cap twin and the minter disagreed, and
...
- `README.md:135` — comparison-table row "Review verdicts are read, not assumed": extend the
  ...
  lookalike namespace.
- `skills/odyssey/SKILL.md:443-449` — the review-gate chain description: the sentence "the nonce
...
change per release, shipped alone** — the repo rule with its precedent at `CHANGELOG.md:577`. Do
...
- **Known, not fixed** — name them; the next audit should not have to find them:
  ...
  - The orchestrator-adversary residual stands unchanged (`skills/odyssey/SKILL.md:449`: the nonce
