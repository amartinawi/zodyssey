...
- Do not fix the adjacent named residuals (new-file lexical-resolution symlink redirect,
  `CHANGELOG.md:213`; the unlocked state writes) — separate releases.
...
path and not its Bash twin" (full account at `CHANGELOG.md:240-242`, including that the suite did
...
(`CHANGELOG.md:190`). This instance is the shape inverted in origin but identical in effect: the
...
- `CHANGELOG.md:198` — move the v0.5.1 *Known, not fixed* bullet into the new version's **Fixed**
  (shape below).
...
with its own precedent at `CHANGELOG.md:151` (a structural gate change "wants its own release and
...
- **Known, not fixed** — name them; the next audit should not have to find them:
  ...
  - Pre-existing residuals this change does not touch: the new-file lexical fallback / symlink
    redirect (`CHANGELOG.md:213`), the unlocked state writes.
