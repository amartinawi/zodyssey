...
Already named in the ledger: `CHANGELOG.md:408` (v0.5.1, *Known, not fixed*) — "The Edit-path scope
...
- Do not fix the adjacent named residuals (new-file lexical-resolution symlink redirect,
  `CHANGELOG.md:409`; the unlocked state writes) — separate releases.
...
path and not its Bash twin" (full account at `CHANGELOG.md:452`, including that the suite did
...
(`CHANGELOG.md:386`). This instance is the shape inverted in origin but identical in effect: the
...
- `CHANGELOG.md:408` — move the v0.5.1 *Known, not fixed* bullet into the new version's **Fixed**
  (shape below).
...
with its own precedent at `CHANGELOG.md:347` (a structural gate change "wants its own release and
...
- **Known, not fixed** — name them; the next audit should not have to find them:
  ...
  - Root-equality edge: a target exactly equal to the run repo or `PROJECT_DIR` returns `rel: ""`
   ...
   - Pre-existing residuals this change does not touch: the new-file lexical fallback / symlink
     redirect (`CHANGELOG.md:409`), the unlocked state writes.
