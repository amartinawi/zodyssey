...
Already named in the ledger: `CHANGELOG.md:534` (v0.5.1, *Known, not fixed*) — "The Edit-path scope
...
- Do not fix the adjacent named residuals (new-file lexical-resolution symlink redirect,
  `CHANGELOG.md:547`; the unlocked state writes) — separate releases.
...
path and not its Bash twin" (full account at `CHANGELOG.md:590`, including that the suite did
...
with its own precedent at `CHANGELOG.md:473` (a structural gate change "wants its own release and
...
- **Known, not fixed** — name them; the next audit should not have to find them:
  ...
  - Root-equality edge: a target exactly equal to the run repo or `PROJECT_DIR` returns `rel: ""`
   ...
   - Pre-existing residuals this change does not touch: the new-file lexical fallback / symlink
     redirect (`CHANGELOG.md:547`), the unlocked state writes.
