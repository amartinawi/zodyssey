...
Already named in the ledger: `CHANGELOG.md:165` (v0.5.1, *Known, not fixed*) — "The Edit-path scope
...
- Do not fix the adjacent named residuals (new-file lexical-resolution symlink redirect,
  `CHANGELOG.md:166`; the unlocked state writes) — separate releases.
...
path and not its Bash twin" (full account at `CHANGELOG.md:207-209`, including that the suite did
...
(`CHANGELOG.md:143`). This instance is the shape inverted in origin but identical in effect: the
...
- **Known, not fixed** — name them; the next audit should not have to find them:
  ...
  - Pre-existing residuals this change does not touch: the new-file lexical fallback / symlink
    redirect (`CHANGELOG.md:166`), the unlocked state writes.
