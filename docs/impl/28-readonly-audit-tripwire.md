...
The external auditor is spawned read-only at three sites — post-done (`skills/odyssey/scripts/consult.mjs:1284-1298`), plan-audit (`:361-370`), and `runSingleAudit` (`:620-631`) — always with `--permission-mode plan --allowedTools ""` (the audit-2026-08-01 gap-#8 injection defense). But the read-only-ness is a **promise with no witness**: nothing before or after the multi-minute spawn checks that the tree is unchanged, and the code knows it — the spawn comment calls the flags the mitigation, while `consult.mjs:1020-1021` (per the study) treats "did the auditor touch anything" as unasked. The only mid-window detection that exists is HEAD-movement (`consult.mjs:1343-1351`, a stderr warning; `audit_head` recorded at `:1534`), which sees committed history only — a window that dirties the working tree or drops untracked files passes silently.
