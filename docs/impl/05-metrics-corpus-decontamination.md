...
the eval dir exists only because `scripts/install.mjs:1018` created it — a hermetic test or a
...
- **Anything assuming one corpus file.** The exact readers, checked against the 2026-08-16 tree:
  ...
  the shift to a code change. `skills/odyssey/scripts/harness.mjs:43,371` — updated in this
  change to report both lanes. `skills/odyssey/scripts/run-report.mjs:244` — the manual-append
...
- `docs/MEASUREMENT.md:173` — the pipeline diagram's `APPEND to eval/results.jsonl ◄── the trend
...
- **Known, not fixed** — name them; the next audit should not have to find them:
  ...
  - `run-report.mjs:244`'s manual-append footer names `results.jsonl` only — a human
