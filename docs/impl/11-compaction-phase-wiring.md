...
**zero code callers**. Every remaining reference is doc prose: `skills/odyssey/SKILL.md:184` ("MAY
run `scripts/compact.mjs <repo> <slug>`" — an instruction addressed to the conductor, inside the
phase-6 OPTIONAL COMPACTION box at `:173-180`), `skills/odyssey/SKILL.md:244` ("the **optional**
`_compact-brief.md`"), `skills/odyssey/references/scripts.md:17` ("OPTIONAL pre-final-wave
notepad compactor"), `README.md:67`, `README.md:90`, `README.md:291`, `CHANGELOG.md:1055`. The
...
The consequence, in the repo's own framing (`skills/odyssey/SKILL.md:232-244`): notepads are the
...
3. **Ceremony without mechanism.** This is the change's own subject: a "MAY run" sentence
   addressed to a conductor (`SKILL.md:184`) becomes a transition invoke with a printed signal —
...
- **Before the fix (current HEAD): no brief, ever.** `node skills/odyssey/scripts/compact.mjs
  ...
  prose at `SKILL.md:184`. F1-F4 dispatch consumes the full doc set the brief was designed to
  replace.
...
`references/scripts.md:17`, `SKILL.md:183-190`, `:243` — a doc-code drift manufactured by this
...
- `skills/odyssey/SKILL.md:184-190` — the phase-6 OPTIONAL COMPACTION box: from "you MAY run" to
...
- `skills/odyssey/SKILL.md:244` — the "load-bearing working memory" paragraph: "optional
  `_compact-brief.md`" becomes "auto-derived at final entry above the size threshold (additive —
  sources never modified)".
- `skills/odyssey/SKILL.md:446-450` region — the env-var list gains
