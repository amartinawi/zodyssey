...
**zero code callers**. Every remaining reference is doc prose: `skills/odyssey/SKILL.md:185` ("MAY
run `scripts/compact.mjs <repo> <slug>`" — an instruction addressed to the conductor, inside the
phase-6 OPTIONAL COMPACTION box at `:173-180`), `skills/odyssey/SKILL.md:245` ("the **optional**
`_compact-brief.md`"), `skills/odyssey/references/scripts.md:17` ("OPTIONAL pre-final-wave
notepad compactor"), `README.md:67`, `README.md:90`, `README.md:291`, `CHANGELOG.md:1078`. The
...
The consequence, in the repo's own framing (`skills/odyssey/SKILL.md:233-245`): notepads are the
...
3. **Ceremony without mechanism.** This is the change's own subject: a "MAY run" sentence
   addressed to a conductor (`SKILL.md:185`) becomes a transition invoke with a printed signal —
...
- **Before the fix (current HEAD): no brief, ever.** `node skills/odyssey/scripts/compact.mjs
  ...
  prose at `SKILL.md:185`. F1-F4 dispatch consumes the full doc set the brief was designed to
  replace.
...
compaction goes stale the moment it auto-fires — `README.md:67`, `README.md:90`, `README.md:291`,
`references/scripts.md:17`, `SKILL.md:184-191`, `:244` — a doc-code drift manufactured by this
...
- `skills/odyssey/SKILL.md:185-191` — the phase-6 OPTIONAL COMPACTION box: from "you MAY run" to
...
- `skills/odyssey/SKILL.md:245` — the "load-bearing working memory" paragraph: "optional
  `_compact-brief.md`" becomes "auto-derived at final entry above the size threshold (additive —
  sources never modified)".
- `skills/odyssey/SKILL.md:452-456` region — the env-var list gains
...
- `README.md:67`, `README.md:90` — the diagram and phase-list "(optional)" qualifiers update to
  "automatic for large runs".
- `README.md:291` — the comparison-table row ("Notepad compaction (#8)") states the transition
