...
**zero code callers**. Every remaining reference is doc prose: `skills/odyssey/SKILL.md:157` ("MAY
...
The consequence, in the repo's own framing (`skills/odyssey/SKILL.md:201-211`): notepads are the
...
- **Before the fix (current HEAD): no brief, ever.** `node skills/odyssey/scripts/compact.mjs
  ...
  prose at `SKILL.md:157`. F1-F4 dispatch consumes the full doc set the brief was designed to
  replace.
...
compaction goes stale the moment it auto-fires — `README.md:59`, `:81`, `:253`,
`references/scripts.md:17`, `SKILL.md:156-163`, `:211` — a doc-code drift manufactured by this
...
- `skills/odyssey/SKILL.md:156-163` — the phase-6 OPTIONAL COMPACTION box: from "you MAY run" to
...
- `skills/odyssey/SKILL.md:211` — the "load-bearing working memory" paragraph: "optional
  `_compact-brief.md`" becomes "auto-derived at final entry above the size threshold (additive —
  sources never modified)".
- `skills/odyssey/SKILL.md:385-388` region — the env-var list gains
