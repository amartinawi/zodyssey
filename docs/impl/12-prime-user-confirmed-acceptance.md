...
THE ORIGINAL TASK (`consult.mjs:1006`, `:1004`) and the stamp is a first-line HTML comment in what is
...
- **Byte-exact consumers of `<slug>.task.md`.** `consult.mjs` reads the file as THE ORIGINAL TASK
  for scope-fidelity judgment (`consult.mjs:1006`, `:1004`); a first-line HTML comment is additive to
