...
run reaches `done|audited`, `skills/odyssey/scripts/set-phase.mjs:217-226` spawns the cached
...
3. **Export failure never breaks the run.** Endpoint unreachable, non-2xx response, malformed
   ...
   (`skills/odyssey/scripts/set-phase.mjs:217`, "never fail the phase transition on a report
...
- Do not alter the results.jsonl record shape, the append at
  `skills/odyssey/scripts/set-phase.mjs:225-226`, or run-report's exit contract.
...
the run); the results.jsonl append at `skills/odyssey/scripts/set-phase.mjs:225-226` still fires,
...
- **Run close gains one outbound HTTP request and one bounded child spawn.** Opt-in by env var;
  ...
  `skills/odyssey/scripts/set-phase.mjs:205-211` and `:217-228` is the precedent: best-effort
...
- Release mechanics per `docs/DEVELOPMENT.md`: CHANGELOG → tag → `scripts/install.mjs`, then
  **re-Get/Update the plugin so the marketplace cache picks up the scripts** — the emitter is
  spawned from the cache path exactly like run-report is (`set-phase.mjs:220-224`), so a fix that
