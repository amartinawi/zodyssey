...
the one-security-change-per-release cadence, `CHANGELOG.md:169` — grouped adjacently with 01/03
...
originated as the author's personal low-friction setup (`CHANGELOG.md:756`).
...
- The Bash gate has been deleted **twice** — v0.1.1 (5c99927) and v0.2.0 (e57b01b) — and both
  ...
  in four documentation locations and **zero lines of executable code**" (`CHANGELOG.md:531`), and
  the v0.1.2 restore entry (`CHANGELOG.md:710`) carries the post-mortem note that the first fix
  "did not hold".
...
- Do not batch this into 01's or 03's release in the CHANGELOG — one security change per release
  (`CHANGELOG.md:169`: a structural gate change "wants its own release and its own paired run").
...
(`docs/INSTALL.md:152`, `CHANGELOG.md:756`). After this change, every ungated run leaves: a ledger
...
deletions (`skills/odyssey/hooks/pre-tool.bash-gate.test.mjs:3-11`, `CHANGELOG.md:531`) were both
...
(`CHANGELOG.md:710`) was not a logic bug at all — it was this variable's silent ambient presence in
...
path by which the second deletion shipped unnoticed (`CHANGELOG.md:550`: audits review the diff in
...
`CHANGELOG.md:169`. Do NOT batch queue items 01 or 03 into this release even though all three touch
`skills/odyssey/hooks/pre-tool.mjs`.
