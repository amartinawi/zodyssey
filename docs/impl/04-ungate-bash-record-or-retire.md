...
the one-security-change-per-release cadence, `CHANGELOG.md:577` — grouped adjacently with 01/03
...
the env table at `docs/INSTALL.md:163`, and the README comparison row at `README.md:133`. It
originated as the author's personal low-friction setup (`CHANGELOG.md:1124`).
...
- The Bash gate has been deleted **twice** — v0.1.1 (5c99927) and v0.2.0 (e57b01b) — and both
  ...
  in four documentation locations and **zero lines of executable code**" (`CHANGELOG.md:959`), and
  the v0.1.2 restore entry (`CHANGELOG.md:1138`) carries the post-mortem note that the first fix
  "did not hold".
...
- *For recording (and against retiring):* retirement breaks a legitimate, documented, four-anchor
  affordance (`scripts/install.mjs:975`, `docs/INSTALL.md:163`, `README.md:133`,
...
3. The run report surfaces the count. `skills/odyssey/scripts/run-report.mjs` reads the ledger if
   ...
   (`skills/odyssey/scripts/run-report.mjs:115-122`, report field at `:167`). Because
   `skills/odyssey/scripts/set-phase.mjs:478-498` already pipes `run-report --json` into the trend
...
existing `run-report --json` pipe (`skills/odyssey/scripts/set-phase.mjs:478-498`). The docs listed
...
- Do not batch this into 01's or 03's release in the CHANGELOG — one security change per release
  (`CHANGELOG.md:577`: a structural gate change "wants its own release and its own paired run").
...
(`docs/INSTALL.md:163`, `CHANGELOG.md:1124`). After this change, every ungated run leaves: a ledger
...
via the existing auto-append (`skills/odyssey/scripts/set-phase.mjs:478-498`). No exit code changes
...
deletions (`skills/odyssey/hooks/pre-tool.bash-gate.test.mjs:3-11`, `CHANGELOG.md:957`) were both
...
(`CHANGELOG.md:1124`) was not a logic bug at all — it was this variable's silent ambient presence in
...
path by which the second deletion shipped unnoticed (`CHANGELOG.md:944`: audits review the diff in
...
- `skills/odyssey/SKILL.md:451-458` — the "Environment overrides (documented)" list omits
...
- `README.md:133` — comparison-table row: "…Secure by default; `ZODYSSEY_UNGATE_BASH=1` disables"
...
`CHANGELOG.md:577`. Do NOT batch queue items 01 or 03 into this release even though all three touch
`skills/odyssey/hooks/pre-tool.mjs`.
...
declaration against hook-witnessed loads (`skills/odyssey/references/scripts.md:23`), so a
