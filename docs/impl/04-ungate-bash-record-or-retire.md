...
the one-security-change-per-release cadence, `CHANGELOG.md:464` — grouped adjacently with 01/03
...
originated as the author's personal low-friction setup (`CHANGELOG.md:991`).
...
- Do not batch this into 01's or 03's release in the CHANGELOG — one security change per release
  (`CHANGELOG.md:464`: a structural gate change "wants its own release and its own paired run").
...
(`docs/INSTALL.md:163`, `CHANGELOG.md:991`). After this change, every ungated run leaves: a ledger
...
deletions (`skills/odyssey/hooks/pre-tool.bash-gate.test.mjs:3-11`, `CHANGELOG.md:824`) were both
...
(`CHANGELOG.md:991`) was not a logic bug at all — it was this variable's silent ambient presence in
...
`CHANGELOG.md:464`. Do NOT batch queue items 01 or 03 into this release even though all three touch
`skills/odyssey/hooks/pre-tool.mjs`.
