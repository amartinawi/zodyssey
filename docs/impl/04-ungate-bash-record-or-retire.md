...
the one-security-change-per-release cadence, `CHANGELOG.md:27` — grouped adjacently with 01/03
...
- The Bash gate has been deleted **twice** — v0.1.1 (5c99927) and v0.2.0 (e57b01b) — and both
  ...
  in four documentation locations and **zero lines of executable code**" (`CHANGELOG.md:389`), and
  the v0.1.2 restore entry (`CHANGELOG.md:554`) carries the post-mortem note that the first fix
  "did not hold".
...
3. The run report surfaces the count. `skills/odyssey/scripts/run-report.mjs` reads the ledger if
   ...
   `skills/odyssey/scripts/set-phase.mjs:217-226` already pipes `run-report --json` into the trend
...
existing `run-report --json` pipe (`skills/odyssey/scripts/set-phase.mjs:217-226`). The docs listed
...
- Do not batch this into 01's or 03's release in the CHANGELOG — one security change per release
  (`CHANGELOG.md:27`: a structural gate change "wants its own release and its own paired run").
...
4. `node --test skills/odyssey/hooks/pre-tool.bash-gate.test.mjs` — expected exit **0**. Mandatory
   ...
   the Bash gate.
5. `node scripts/run-tests.mjs` — expected exit **0**. Baseline on arrival: 32/32 suites,
...
(`docs/INSTALL.md:152`, `CHANGELOG.md:614`). After this change, every ungated run leaves: a ledger
...
via the existing auto-append (`skills/odyssey/scripts/set-phase.mjs:217-226`). No exit code changes
...
deletions (`skills/odyssey/hooks/pre-tool.bash-gate.test.mjs:3-11`, `CHANGELOG.md:389`) were both
...
(`CHANGELOG.md:554`) was not a logic bug at all — it was this variable's silent ambient presence in
...
path by which the second deletion shipped unnoticed (`CHANGELOG.md:389`: audits review the diff in
...
`CHANGELOG.md:27`. Do NOT batch queue items 01 or 03 into this release even though all three touch
`skills/odyssey/hooks/pre-tool.mjs`.
