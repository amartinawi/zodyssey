...
1. **The loop's convergence claim is unmeasured.** "The remediation loop converges because
   ...
   (`skills/odyssey/scripts/consult.mjs:1616-1629`) and the report carries only
...
2. **`consult.mjs` — one additive field on every history push.** Before each push, when
   ...
   gaps (`last_gaps` — post-routing/post-refute, `consult.mjs:1607`) against the previous
   ...
   (`consult.mjs:1616-1629`) and the multi-auditor lane (`consult.mjs:797`). First round →
...
- **Never inject prior-round gaps into the next auditor's prompt** — round independence is
  ...
  (`consult.mjs:1340-1374`) gains nothing.
...
- `normalizeConsultVerdict` and `extractRemediationPlan` stay byte-identical
  ...
  only; routing/refute semantics (`consult.mjs:1452-1546`) are not re-derived, consumed
  as-is.
