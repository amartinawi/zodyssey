...
`done|audited`, `skills/odyssey/scripts/set-phase.mjs:478-498` auto-appends a run-report record to
...
(`CHANGELOG.md:535` dated 2026-08-15, entry at `CHANGELOG.md:595`; commit `6b0b428`
...
- Do not make telemetry failure fail anything: run-report's exit contract
  ...
  and the auto-append's best-effort guarantee (`set-phase.mjs:474`, verified untouched) stands.
...
- **Probe C — end-to-end at close.** The fix-run's own `done`/`audited` transition auto-appends a
  ...
  **After: never bare null.** Cache lesson carried from the natural experiment: the auto-append
  executes the CACHED run-report (`set-phase.mjs:481-485`), so the end-to-end direction is only
...
- Release mechanics per `docs/DEVELOPMENT.md`: CHANGELOG → tag → `scripts/install.mjs`, then
  **re-Get/Update the plugin so the marketplace cache picks up the scripts** — the auto-append
  runs the CACHED run-report (`set-phase.mjs:481-485`), and the `truncate-roundto` pair (null at
