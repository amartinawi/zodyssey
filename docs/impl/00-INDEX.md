...
skills/odyssey/scripts/consult.mjs:1274 post-done, :361 plan-audit, :625 multi-auditor pass), and
...
(skills/odyssey/scripts/consult.mjs:1008-1009). The only post-spawn check catches COMMITTED moves
(the HEAD-moved race warning re-reads `rev-parse HEAD`, consult.mjs:1334-1338) — a working-tree
...
`run_start_sha`/`audit_head` (consult.mjs:1521-1523). Deterministic git plumbing only — zero
...
`/orchestrate-consult` audit; rows 30-32 (the open-code-review pair) remain QUEUED for
their own runs.
