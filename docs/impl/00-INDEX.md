...
| 25 | eval-loop-meta-layer | — | **QUEUED (commissioned 2026-08-21).** `docs/impl/25-eval-loop-meta-layer.md` is the build brief. The loop measures and never learns: judge.mjs/run-report.mjs score every run, the corpora hold hundreds of scored outcomes, and nothing feeds back across runs — `recall-corrections.mjs:32` names the missing organ as its own documented FUTURE WORK ("the capture half, not the edit half"). Item 25 builds the edit half the prescribed way: a zero-LLM deterministic miner over the two corpora detecting RECURRING failure classes (criterion-shape families, reject-blocker classes, verify-fail cycles, consult-gap categories; recurrence ≥ 3), writing STAGED PROPOSALS under `.zcode/staging/proposals/` — never a live edit; metis's consult dispatch gains the unapplied-proposals pointer. Applying stays human-approved (the task-observer staging-only model). ISNAD drops stay dropped (chain-scoring, corroboration, R6/R7). Signal-3 capture stays not-closed. | A recurring failure pattern in the corpora becomes a staged, evidence-cited proposal the next consult reads — measurement acquires memory without an opinion layer. |
