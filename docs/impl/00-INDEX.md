...
- Stale anchors: agents/sisyphus-junior.md:93 and docs/ROADMAP.md:188 — both cited regions had drifted. **RESOLVED 2026-08-16.** sisyphus-junior.md:93 was re-anchored (it cited pre-tool.mjs:896-952 for the Bash gate and :807 for the UNGATE hatch; correct values are :1072-1226 and :978, and :807 had become the unrelated `if (rel)` containment escape). ROADMAP.md:188 is valid again — its status addendum was deliberately APPENDED rather than inserted, so every line number in that file is unchanged. Both are now citeable.
