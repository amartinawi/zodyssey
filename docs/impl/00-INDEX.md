...
| 09 | two-arm-eval-baseline | 05 | skills/odyssey/scripts/judge.mjs:278 hardcoded the arm (fixed 2026-08-17 via lib/arm.mjs, row 19 rider; amendment in the 09 brief); skills/odyssey/scripts/harness.mjs:21-23 still has baseline = TODO. The settling experiment must draw from a decontaminated corpus or its first number is poisoned. | Both arms run through `judge.mjs --arm`; the baseline arm is automated. |
...
discovered: in `CHANGELOG.md:562/:696` only `:531` is seen, and in a range followed by prose like
