...
| 28 | readonly-audit-tripwire | — | **QUEUED — brief docs/impl/28-readonly-audit-tripwire.md (candidate C6 promoted, the delegate-skills adaptation study's single VIABLE).** The external auditor spawns read-only at three sites (consult.mjs:1038-1046/:249-256/:502-510) but the promise has no witness: only HEAD-movement is detected (:1091-1099), and a window that dirties the working tree or drops untracked files passes silently — while the harness's own empty-work guard (harness.mjs:272-309) already refuses to trust agents' self-description on exactly these two surfaces. Item 28 inverts that primitive for the auditor: one shared before/after snapshot helper (porcelain+untracked filtered to work paths, HEAD, fail-closed null) wrapping all three spawn sites, recording tri-state `readOnlyViolation` into every consult.history entry beside `audit_head`. Records and surfaces, never adjudicates — a concurrent session can legally commit mid-window (11f3467 did, mid-study). Post-done spawn becomes injectable so the hermetic suite drives all three paths. | Suite 57 → 58 expected (`consult.tripwire.test.mjs`, RED-first); pending the build run. |
...
**Commissioned as row 28** (`docs/impl/28-readonly-audit-tripwire.md`, 2026-08-24) — pending the build run.
