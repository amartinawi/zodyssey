...
|---|---|---|---|---|
| 01 | edit-path-containment-escape | — | **SHIPPED v0.5.3.** Cheapest and most severe open defect; the escape is post-OKAY-only (pre-OKAY targets outside PROJECT_DIR are already blocked at skills/odyssey/hooks/pre-tool.mjs:864). Own release, shipped alone. | No edit path skips the scope gate; the `if (rel)` branch is gone. |
...
| 14 | otel-genai-span-emission | — | Externally blocked: semconv `gen_ai.*` attributes are still "Development" stability (re-fetched 2026-08-23 — unchanged, none Stable; `gen_ai.*` split into the dedicated `semantic-conventions-genai` repo at core v1.42.0, which has no tagged release; docs/OPPORTUNITY-MAP.md:275). Last in the queue. | One run-level span at close (built-ins-only OTLP/JSON, inert when unconfigured); per-dispatch granularity named Known-not-fixed — dispatch timestamps do not exist yet. (Outcome narrowed from "per-dispatch spans" while writing prompt 14, 2026-08-16.) |
...
| 25 | eval-loop-meta-layer | — | **SHIPPED v0.7.0 (2026-08-22).** `docs/impl/25-eval-loop-meta-layer.md` is the build brief. The loop measured and never learned: judge.mjs/run-report.mjs score every run, the corpora hold hundreds of scored outcomes, and nothing fed back across runs — `recall-corrections.mjs:32` pinned the missing organ inside its own FUTURE WORK block; the comment's edit half now exists (recall-corrections stays the capture half, `mine-corrections.mjs` the edit half). Item 25 shipped that edit half the prescribed way: a zero-LLM deterministic miner over the two corpora detecting RECURRING failure classes (criterion-shape families, reject-blocker classes, verify-fail cycles, consult-gap categories; recurrence counted only at ≥ 3 DISTINCT run slugs), writing STAGED PROPOSALS under `.zcode/staging/proposals/` and nothing else — never a live edit; metis's consult dispatch now lists the unapplied proposals as risk-input (one SKILL.md consult-box line; the conductor never runs the miner). Close-out: suite 54 → 55 (`mine-corrections.test.mjs`, hermetic, RED first against the unimplemented miner); the first real invocation left both operator corpora byte-identical (sha256-proven) while staging three evidence-cited consult-gap proposals from real state history. Applying stays human-approved (the task-observer staging-only model). ISNAD drops stay dropped (chain-scoring, corroboration, R6/R7). Signal-3 capture stays not-closed. | A recurring failure pattern in the corpora becomes a staged, evidence-cited proposal the next consult reads — measurement acquires memory without an opinion layer. |
| 26 | audit-vehicle-audited-edge | — | **SHIPPED v0.7.1 (2026-08-22).** Candidate C1 promoted; `docs/impl/26-audit-vehicle-audited-edge.md` is the build brief. The DAG gave `audited` two predecessors only — `done` and `remediate` (skills/odyssey/scripts/set-phase.mjs:93-94) — so a run opened purely to carry an external audit of already-shipped work, which executes nothing and ends at `abandoned`, could never wear the label it earned; and because the terminal auto-append fires on `done`/`audited` only (skills/odyssey/scripts/set-phase.mjs:478), those runs contributed zero trend records — row 18's `external-audit` origin, built to distinguish audited work from self-graded work, never arrived from the one run class that most deserves it. The fix is a single edge — `abandoned → audited` (skills/odyssey/scripts/set-phase.mjs:97) — behind a destination precondition reading only `consult.verdict === "ACCEPT"` (minted by trusted consult.mjs alone; measured 2026-08-22, all 16 runs at `audited` carry one), which also tightens the existing `done → audited` and `remediate → audited` entries; `audited` stays outside FORCEABLE (skills/odyssey/scripts/set-phase.mjs:319), so `--force` still cannot mint the label. Victim on file: `impl-04-audit` — an ACCEPT with zero gaps, parked at `abandoned`, contributing zero corpus records. | Suite 55 → 56 (`set-phase.audit-vehicle.test.mjs`, RED-first on the unmodified DAG — 11 ✗ — then GREEN with zero test edits); 16/16 audited runs satisfy the new precondition; `--force` still cannot mint `audited`. |
| 27 | trend-record-version-stamp | — | **SHIPPED v0.7.2 (2026-08-22).** Candidate C3 promoted; `docs/impl/27-trend-record-version-stamp.md` is the build brief. The emitted trend record carried no version field at all while the only thing that could answer "which plugin version emitted this row?" — the version-named plugin-cache directory — answers it wrong exactly when it matters: the cache path names the last marketplace Get, its contents track the last `--sync-cache` (scripts/install.mjs:37), so the directory name loses on every refresh that outpaces a reinstall, and the terminal auto-append runs the cached emitter. run-report.mjs now stamps `zodyssey_version` into every record, read self-relative from its own `.zcode-plugin/plugin.json` — the copy that actually ran, so the answer follows content, not the directory name — and fail-safe to `null` when that manifest is unreadable, unparseable, or version-less, honouring the best-effort auto-append contract (telemetry can never fail a close; the operator-facing version check stays smoke-gate's). Additive only: one field immediately after `slug`, every other field, ordering, and the text scorecard untouched; legacy records read as version-absent. | Suite 56 → 57 (`run-report.version.test.mjs`, RED-first on the unmodified emitter — 10/14, all four failures naming the absent `zodyssey_version` — then GREEN 14/14 with zero edits to the pre-existing suite, which stays byte-identical); provenance now answers from the emitted record, not the directory name. |
| 28 | readonly-audit-tripwire | — | **SHIPPED v0.7.3 (2026-08-24).** Candidate C6 promoted; `docs/impl/28-readonly-audit-tripwire.md` is the build brief. The external auditor spawns read-only at three sites (consult.mjs:1384-1395/:361-370/:620-631) but the promise had no witness: only HEAD-movement was detected (:1442-1450), and a window that dirties the working tree or drops untracked files passed silently — while the harness's own empty-work guard (harness.mjs:272-309) already refuses to trust agents' self-description on exactly these two surfaces. Item 28 inverts that primitive for the auditor: one shared before/after snapshot helper (porcelain+untracked filtered to work paths, HEAD, fail-closed null) wrapping all three spawn sites, recording tri-state `readOnlyViolation` into every consult.history entry beside `audit_head`. Records and surfaces, never adjudicates — a concurrent session can legally commit mid-window (11f3467 did, mid-study). Post-done spawn became injectable so the hermetic suite drives all three paths. | Suite 57 → 58 (`consult.tripwire.test.mjs`, RED-first, zero edits to pre-existing tests). |
| 29 | remediation-plan-contract | — | **SHIPPED v0.7.4 (2026-09-07).** Run impl-29-remediation-plan-contract; momus OKAY r2 + amendment OKAY r3; F1-F5 pass; external consult ACCEPT round 1 zero gaps (squash-merged as 323a568); `docs/impl/29-remediation-plan-contract.md` is the build brief. The consult REJECT contract is tuned for judge precision while the remediation loop consumes it for convergence: the auditor was told to cap the gap list ("typically ≤5", removed by this row) and park borderline items as advisories (auditor-prompt.md:40-42), but every re-audit round is a FRESH judge that sees nothing of the prior round (consult.mjs:1340-1374) — so every withheld ground is a scheduled future round, the `fix` one-liner carries no runnable proof, and nothing plan-shaped survives (`consult.last_gaps` alone, consult.mjs:1605-1608; the normalizer whitelists its output, lib/verdict-schema.mjs:98-104). Measured 2026-09-06 across this repo's 22 consult-carrying runs: 42 external rounds, 7/22 needed ≥3 (impl-08 and retro-audit-fix-wave-v1 at 5 each); operator reports 8-round non-convergent loops in target repos. Fix = three-part contract: on REJECT the gap list is the COMPLETE rejection surface (borderline-that-could-fail-a-fresh-judge = minor gap, not advisory; trivial-nits bar untouched), each gap carries a single-line runnable `verify`, and an ordered `remediation_plan` persists to `consult.last_remediation_plan` (extractor appended to verdict-schema.mjs; normalizeConsultVerdict byte-identical) — the loop gains a pre-audit verify gate so a failing proof loops back without burning an audit round. Independence preserved: round N+1 still sees nothing from round N; scripts never execute auditor strings (verify runs in the conductor's permissioned Bash lane). | A REJECT arrives with its complete remediation surface — every real ground listed once, each with a runnable proof, ordered — and the loop cannot re-audit past a failing verify. |
| 30 | ocr-routed-capability | — | QUEUED (2026-09-15), hand-landed 2026-09-16 (0390ecd) — **SHIPPED v0.7.5.** Commissioned from the open-code-review adaptation study. `ocr` (alibaba/open-code-review — Apache-2.0, 28k★, npm `@alibaba-group/open-code-review` v1.12.2) is a benchmarked line-precise external code reviewer, and the routing table cannot see it: "review this PR/changeset line-by-line" has no row (skills/odyssey/references/capabilities.md:49 covers the in-session flow), so triage/metis default to generic in-session review — the Task-B class the routing rule (skills/odyssey/SKILL.md:47-58) exists to prevent, and `ocr` clears the external-skill quality gate (SKILL.md:45) with room to spare. Docs-only fix: one additive matrix row + install-story detail paragraph in capabilities.md, one headline-table row in SKILL.md, with the optionality marker (absent `ocr` degrades to today's routes, never a gate). Routing-only by design: `ocr` never joins consult/F2/F4 — the study's REJECTED option (own API-key lifecycle; `CLAUDE_CLI_2` is the existing second-model knob, SKILL.md:459). `docs/impl/30-ocr-routed-capability.md` is the build brief. | Routing rows live (capabilities.md matrix + detail, SKILL.md headline); docs-only, no behavior change. |
| 31 | project-review-rules | — | QUEUED (2026-09-15), hand-landed 2026-09-16 (0390ecd) — **SHIPPED v0.7.5.** The study's second VIABLE adaptation: the audit rubric is one-size for every repo (skills/odyssey/references/auditor-prompt.md:26-38) — a repo cannot declare its own defect classes to any review lane, and a rules section without a contract change would be unusable (the must-not at auditor-prompt.md:46-52 bars "requirements not in the plan"). Fix = OCR's rule.json core transplanted zero-dependency: a `.zcode-review-rules.json` at the repo ROOT (not `.zcode/` — gitignored run-artifact territory; read from the working tree whether or not it is tracked; meant to be committed), glob-matched against changed files by a hand-rolled matcher, injected as DATA between THE PLAN and THE DIFF in the post-done prompt only (skills/odyssey/scripts/consult.mjs:1356 seam; the multi-auditor prompt carries no diff at all — consult.mjs:602-620 — and plan-audit judges a plan, consult.mjs:195-206), capped (≤8 rules / ≤200 chars / ≤4KB), absent file → prompt BYTE-IDENTICAL, malformed → warn+skip (fail-open, never a gate). One legitimizing line appended to auditor-prompt.md (matched rules are project requirements, still subject to the precision bar, auditor-prompt.md:121-128); the F2 dispatch detail (skills/odyssey/references/capabilities.md:103 — SKILL.md's only F2 text is the ASCII phase-6 box, not a prose seam) gains a rules-file pointer. `docs/impl/31-project-review-rules.md` is the build brief. | consult.test 119 → 139 across the audit loop (r1)-(r10): present→injected between PLAN and DIFF, absent→byte-identical, malformed→warn+skip, caps enforced incl. the linear DP matcher (audit rounds 2-3) and newline collapse (round 2); auditor legitimizing line + F2 pointer. |
| 32 | gap-lifecycle-ledger | — | QUEUED (2026-09-15), hand-landed 2026-09-16 (0390ecd) — **SHIPPED v0.7.5.** The study's third VIABLE adaptation: the remediation loop's convergence claim ("each round shrinks the gap list", skills/odyssey/SKILL.md:343-345) is measured only as a round count (run-report.mjs:184) — every round is a FRESH judge, so a re-raised ground is indistinguishable from new information (row 29's 8-round operator evidence), and the filter-miss signal (SKILL.md:327-328) is prose with no key to match on. Fix = OCR session-compare primitives ported at zero LLM cost: new `lib/gap-ledger.mjs` (gapKey = category + whitespace-collapsed issue; multiset compareGaps → new/persisting/resolved), one additive `gap_delta` field at both history-push sites (consult.mjs:1616-1629 post-done; consult.mjs:797 multi-auditor), and run-report emits `consult_gap_lifecycle` (incl. `max_persisting_streak` — the honest non-convergence signal), `recurred_gaps_from_prior_runs` (the per-finding twin of item 25's class recurrence; miner consumption stays 25's extension), and `open at close` (terminal-REJECT kept gaps are never "resolved" — OCR's never-congratulate NotReviewed discipline). Advisory only; round N+1's prompt still gains nothing from round N. Suite 59 → 60 (new suite file). `docs/impl/32-gap-lifecycle-ledger.md` is the build brief. | gap-ledger suite 16/16 (suite 59 → 60); gap_delta at both push sites; run-report consult_gap_lifecycle/open-at-close/recurrence with distinct-key counting (version suite 23/23); real-data smoke over retro-audit-fix-wave-v1: new 2→1→1→0. |
| 33 | research-deliverable-contract | — | QUEUED (2026-09-16), hand-landed same-day — **SHIPPED v0.7.5.** Commissioned from the hyperresearch adaptation study: research deliverables have no structure contract — a thematically-organized answer to a per-entity question passes every gate with the rubric unmet, and period-pinned asks ("FY 2023") are the top silent miss. Fix = the plan gains `## Deliverable contract` for research-kind runs (register/format/tier levers — analyze default, user directive wins, tier-up-when-uncertain; ordered literal headings; atomic items incl. period-pinned figures + a coverage note) with the house mechanical/judgment split: parse-plan lints the section's SHAPE when present (vacuous contract fails exit 6, the routing-token precedent at skills/odyssey/scripts/parse-plan.mjs:70-86; absence lints unchanged — kind is not the parser's to know), momus REJECTs a research plan without it, and auditor-prompt.md gains the judge line as an EOF append (zero pin movement above). Headings become grep-able verify criteria. `docs/impl/33-research-deliverable-contract.md` is the commission + implementation record. | parse-plan 36 → 38 incl. the round-5 menu-resolution fix; momus Lens-2 presence blocker + auditor EOF judge line live; externally audited ACCEPT r6 (vehicle retro-audit-rows-30-34). |
| 34 | research-discipline-pack | — | QUEUED (2026-09-16), hand-landed same-day — **SHIPPED v0.7.5.** The hyperresearch study's prompt-layer trio, the research-shaped twin of row 20's span-citation: research dispatches carry the canonical question VERBATIM block-quoted (hyperresearch's "gospel" invariant — paraphrase is where scope drift enters); librarian findings carry verbatim quoted passages + permalinks with `contested:` markers and syndication awareness ("five reprints argue with the weight of one source"); explore answers cite path:line spans; research notepads end with a one-line `Answers the question: yes|partial|no` drift check; and synthesis sub-agents must emit a Tensions section (claim vs claim, sources, adopted resolution + why — contested pairs are never silently merged). Prompt-layer only: no hooks, no scripts, no state, suite unchanged. `docs/impl/34-research-discipline-pack.md` is the commission + implementation record. | Prompt contracts live in metis/librarian/explore + SKILL.md (verbatim dispatch, Tensions section, drift check, span citation); externally audited ACCEPT r6 in the same vehicle. |
...
terminal auto-append fires on `done` and `audited` only (skills/odyssey/scripts/set-phase.mjs:478),
...
(skills/odyssey/scripts/set-phase.mjs:319), so an ungated edge would let `--force` mint the
...
a continued form was never discovered: in `CHANGELOG.md:959/:1060` only the `:927` half was ever
...
**C3 — the plugin cache directory no longer identifies its contents — SHIPPED as row 27 (v0.7.2); `docs/impl/27-trend-record-version-stamp.md` is the build brief.** The cache path names the
...
**C6 — the read-only auditor's write behavior is a promise, never verified — SHIPPED as row 28 (v0.7.3); `docs/impl/28-readonly-audit-tripwire.md` is the build brief.** Every external
auditor spawn is pinned read-only at the flag level (`--permission-mode plan --allowedTools ""` —
skills/odyssey/scripts/consult.mjs:1385 post-done, :361 plan-audit, :622 multi-auditor pass), and
...
(skills/odyssey/scripts/consult.mjs:1110-1111). The only post-spawn check catches COMMITTED moves
(the HEAD-moved race warning re-reads `rev-parse HEAD`, consult.mjs:1442-1450) — a working-tree
write by a "read-only" auditor is undetectable, and the tests stub the spawn entirely
(consult.mjs:570-571), so nothing anywhere observes actual write behavior. The detection
...
`run_start_sha`/`audit_head` (consult.mjs:1623-1625). Deterministic git plumbing only — zero
...
verifies the external-audit lane none of them touches); open C5 (00-INDEX.md:225-230 — dry-run
...
study (mechanism D-read-only-tripwire; docs/DELEGATE-REVIEW.md); the brief is docs/impl/28-readonly-audit-tripwire.md.

**Commissioned as row 28** (`docs/impl/28-readonly-audit-tripwire.md`, 2026-08-24) — shipped v0.7.3.

**Commissioned as row 29** (`docs/impl/29-remediation-plan-contract.md`, 2026-09-06) — **shipped v0.7.4** (run impl-29-remediation-plan-contract 2026-09-07:
suite 59→59 with the two declared test appends; consult.test 119/119 incl. the seven (p1)-(p7)
persistence cases; anchors 754/754 after two fixed-order passes; external consult ACCEPT round 1
zero gaps; the section rode as `## [Unreleased]` in-run per the mid-run amendment — the v0.7.4
heading and manifest bumps were minted together at the release cut). Commissioned directly
from the consult-round-cost scoping session (operator-reported 8-round non-convergent
`/orchestrate-consult` loops; no C-candidate round — the evidence and the five-surface overlap
reasoning are in the brief's What-is-broken section).

**Commissioned as rows 30-32** (2026-09-15) — the open-code-review adaptation study. Studied
in-session (the adaptation map with VIABLE/DUPLICATE/REJECTED verdicts per mechanism lives in
the session memory, not a repo doc); the three VIABLE adaptations are commissioned above in
recommended execution order — **30** (docs-only, cheapest) → **31** → **32**. Rows 31 and 32
both edit consult.mjs at different seams (the post-done prompt assembly vs the history-push
sites), one per release, either order valid; no hard edge between them, and neither depends on
anything open — row 29 (shipped v0.7.4) is the only substrate either consumes. Duplicate
verdicts, NOT commissioned: the false-positive filter task (v0.7.4's gap-refute is the same
asymmetric-loss idea, independently invented), the structured verdict (row 29's contract is
richer), effort-preset rounds (`max_rounds` exists), snippet-anchored comment positioning
(ZOdyssey emits no line-anchored comments; the principle is already house style), and
`--background` context injection (the auditor already receives plan + original task).
Rejected: spawning `ocr review` as a consult auditor lane (own API-key lifecycle the pipeline
cannot own; `CLAUDE_CLI_2` is the existing second-model knob) and the session viewer UI
(product-scope expansion). Suite baseline at commissioning: 59/59
(`node scripts/run-tests.mjs`, 2026-09-15, 131s).

**Commissioned as rows 33-34** (2026-09-16) — the hyperresearch adaptation study
(jordan-gibbs/hyperresearch: a 16-step-skill deep-research pipeline for Claude Code; MIT,
~3.4k stars). Studied in-session (adaptation map in the session memory). Four VIABLE
adaptations commissioned as TWO hand-landed items, implemented same-day and riding
`## [Unreleased]`: **33** the research deliverable contract (atomic-item decomposition +
ordered heading contract + coverage note + register/format/tier levers — parse-plan shape
lint + momus presence blocker + auditor EOF judge line) and **34** the research discipline
pack (verbatim-question dispatch, verbatim-quote/contested/syndication evidence contract
for librarian, path:line spans for explore, notepad drift check, synthesis Tensions
section). Duplicate verdicts NOT commissioned: on-demand step-skills (=references/
routing), resumable manifests + ship gate (=checkpoints + final wave + regression gate),
adversarial critics (=momus/oracle/external consult — ours is the stronger separate-process
shape), untrusted-source fencing (=consult DATA framing). Rejected: the 16-agent roster +
per-agent model maps, vault/SQLite/embeddings machinery (memory MCP + docs/ are the
stores), PageRank/retraction/open-access/browser-escalation machinery, dissertation tier,
MCP server. Execution order: both landed together before the operator's
`/orchestrate-consult` audit; rows 30-32 were hand-landed the next day (0390ecd,
2026-09-16) alongside this pair — shipped together as v0.7.5.
