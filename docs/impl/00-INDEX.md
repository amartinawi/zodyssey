...
a continued form was never discovered: in `CHANGELOG.md:936/:1037` only the `:904` half was ever
...
skills/odyssey/scripts/consult.mjs:1199 post-done, :360 plan-audit, :624 multi-auditor pass), and
...
(skills/odyssey/scripts/consult.mjs:940-941). The only post-spawn check catches COMMITTED moves
(the HEAD-moved race warning re-reads `rev-parse HEAD`, consult.mjs:1259-1263) — a working-tree
write by a "read-only" auditor is undetectable, and the tests stub the spawn entirely
(consult.mjs:569-570), so nothing anywhere observes actual write behavior. The detection
...
`run_start_sha`/`audit_head` (consult.mjs:1439-1441). Deterministic git plumbing only — zero
