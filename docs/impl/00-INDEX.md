...
| 09 | two-arm-eval-baseline | 05 | skills/odyssey/scripts/judge.mjs:176 hardcodes the arm; skills/odyssey/scripts/harness.mjs:19 still has baseline = TODO. The settling experiment must draw from a decontaminated corpus or its first number is poisoned. | Both arms run through `judge.mjs --arm`; the baseline arm is automated. |
