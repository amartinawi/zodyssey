...
(`CHANGELOG.md:453`, `CHANGELOG.md:618`) — is still reachable from inside a normally-approved run,
by changing tool a second time. This is the "one path but not its twin" class
(`CHANGELOG.md:187-197`) at triplet scale: fixed on Edit, fixed on Bash, open on everything else.
...
- **Not covered, and stated rather than hidden:** a non-native tool can still write ordinary repo
  ...
  binding (`CHANGELOG.md:150`). Name it in the CHANGELOG under *Known, not fixed*.

## The class it closes

**"A guard added to one path and not its twin."** The repo's most persistent shape:
`CHANGELOG.md:187-197` documents an entire release of them, and v0.5.0's own re-verification found
...
security change per release (`CHANGELOG.md:91`).
