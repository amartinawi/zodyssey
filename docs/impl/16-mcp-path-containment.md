...
(`CHANGELOG.md:531`, `CHANGELOG.md:696`) — is still reachable from inside a normally-approved run,
by changing tool a second time. This is the "one path but not its twin" class
(`CHANGELOG.md:265-275`) at triplet scale: fixed on Edit, fixed on Bash, open on everything else.
...
`CHANGELOG.md:265-275` documents an entire release of them, and v0.5.0's own re-verification found
...
security change per release (`CHANGELOG.md:169`).
