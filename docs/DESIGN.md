...
| `PostToolUse` | `Edit\|Write\|MultiEdit` | **Post-edit lint vs first-touch baseline (v0.6.4):** runs the repo's own `lint_cmd` on the edited file after the edit and injects `decision: "block"` only when the diagnostics are NEW to the edit — `pre-tool.mjs` captured the file's lint state at the run's first touch (side-file `.zcode/state/<slug>.lint-baseline.json`, frozen per target, atomic write). Pre-existing noise, timed-out/unspawnable lints, and runs predating the change record `inert` and never block; a block's reason names the target. No `lint_cmd` (`scripts.lint` absent) → the whole mechanism is inert. |
