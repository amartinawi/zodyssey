...
ZOdyssey is a code-enforced orchestration pipeline for ZCode (for now — the pattern is harness-portable): the review gate, executor scope isolation, and parallel dispatch caps are enforced by hooks, not prompt conventions. The pipeline shape — prime → triage → consult → plan → review → execute → verify → final-wave — is the shape multi-agent orchestrators already share; the delta is that the load-bearing invariants are checked by code instead of trusted to the LLM's good behavior, from plan-review onward.
...
| **No pass-to-pass regressions** | not enforced | the pass-to-pass property, wired both sides as of **v0.6.17** (queue item 24). The suite is baselined at execute entry (`set-phase.mjs:363`, idempotent; `record-review.mjs` captures the same on its OKAY advance), and **the `done` transition itself runs `regression-gate.mjs --check`** (`set-phase.mjs:305`) over the exact tree the final wave judged, re-reads the state, then refuses on `regressed` and `toolchain-drift` (`set-phase.mjs:132`, `:138`) — the invoke half this row shipped without until item 24. An already-red suite is never blamed on the run; no toolchain degrades to `inert`, never a block |
| **Imports resolve** | not enforced | `check-imports.mjs` flags packages in neither the manifest nor `node_modules`, offline, exiting 9. **Wired both sides in v0.6.0** (queue item 02): invoked at verify entry (`set-phase.mjs:404`), recorded to `state.imports`, consumed by a `done` refusal on `status === "unresolved"` (`set-phase.mjs:153`). `inert` and a missing lane still pass, so it cannot wedge a repo it can't evaluate |
...
| **Every trend record names its emitter** (v0.7.2) | n/a | each emitted record carries `zodyssey_version`, read self-relative from the executing copy's own `.zcode-plugin/plugin.json` — the copy that actually ran, whose content follows `--sync-cache` — fail-safe to `null` (unreadable, unparseable, or version-less) so telemetry can never fail a close. The version-named cache directory tracks the last marketplace *Get* and answered "which version emitted this row?" wrong exactly when a content refresh outpaced a reinstall; provenance now answers from the immutable record, not the mutable directory name. Pinned by `run-report.version.test.mjs` (suite 56 → 57) |
| **The external auditor's read-only window is verified, not promised** (v0.7.3) | not enforced | every external-auditor spawn window (post-done, plan-audit, multi-auditor) wraps snapshot→spawn→snapshot→compare: `workTreeSnapshot` does exactly two argv-only git reads (HEAD + porcelain with untracked, filtered to work paths by the copied harness exclusion set), `compareWorkTree` yields the tri-state `readOnlyViolation` — `true` on any work-path change or HEAD move, `null` fail-closed on unreadable git — recorded beside `audit_head` in consult history and stderr-warned naming both possible causes (the auditor OR a concurrent session), never mutating a verdict, an exit code, or triggering a rerun. Closes candidate C6 (the delegate-skills adaptation study's single VIABLE verdict). Pinned by `consult.tripwire.test.mjs` (suite 57 → 58) |
...
Two later additions grow this lane. Since **v0.7.1**, an **audit vehicle** — a run opened purely to carry an external audit of already-shipped work — can reach `audited` directly from `abandoned` through a consult-ACCEPT-gated edge (`--force` can never mint the label), so externally-audited work now contributes its `external-audit`-labelled trend record instead of vanishing at `abandoned`. And since **v0.7.0**, the corpora this gate produces feed back: `mine-corrections.mjs` deterministically detects *recurring* failure classes across runs and stages evidence-cited proposals under `.zcode/staging/proposals/` — never a live edit — which the next consult's metis dispatch reads as risk input. Measurement acquired memory without an opinion layer.
...
|---|---|
| Notepad compaction (#8) | `scripts/compact.mjs` — auto-invoked at `final` entry above `AUTO_COMPACT_MIN_LINES` (inert below; `ZODYSSEY_NO_AUTO_COMPACT=1` skips); concatenates notepads into one brief the F1–F5 reviewers read instead of the full doc set. Deterministic, $0, never mutates the sources (asserted by test) |
...
| **REJECT remediation contract** (v0.7.4) | auditor-prompt.md + consult.mjs — on REJECT the gap list is the round's complete rejection surface (borderline grounds become minor gaps, not advisories — the next round is a fresh judge), every gap carries a single-line runnable `verify`, and an ordered `remediation_plan` persists to `consult.last_remediation_plan` with positional remap past the gap-refute filter; the remediation loop runs a pre-audit verify gate — a failing verify loops back without spending an audit round, and scripts never execute auditor strings |
| **Research deliverables carry a contract** (row 33) | research-kind plans carry a `## Deliverable contract` — register/format/tier levers (analyze default, user directive wins, tier-up-when-uncertain), an ordered list of literal headings, and atomic items incl. period-pinned figures with primary source named + a coverage note. `parse-plan --lint` refuses a vacuous shape (the routing-token precedent); momus rejects a research plan without it; the external auditor judges the finished deliverable heading-by-heading. Headings become grep-able acceptance criteria |
| **Per-project review rules** (row 31) | consult.mjs + auditor-prompt.md — a `.zcode-review-rules.json` at the repo root (read from the working tree whether or not it is tracked; meant to be committed) is glob-matched against the changed files and injected as a DATA section into the post-done audit prompt (≤8 rules / ≤200 chars / ≤4KB; absent file → byte-identical prompt; malformed → warn + skip, never a gate); matched rules are project requirements, still subject to the precision bar |
| **Gap lifecycle ledger** (row 32) | lib/gap-ledger.mjs + consult.mjs + run-report.mjs — deterministic finding identity (category + whitespace-collapsed issue) across audit rounds; every history entry from round 2 on carries `gap_delta` (new/persisting/resolved + persisting_keys), and run-report emits `consult_gap_lifecycle` (incl. `max_persisting_streak`), `open at close` (terminal-REJECT kept gaps are never "resolved"), and `recurred_gaps_from_prior_runs`. Advisory only — nothing gates on it, and round N+1's prompt still receives nothing from round N |
| **Research discipline pack** (row 34) | research dispatches carry the canonical question VERBATIM block-quoted (never a paraphrase); librarian findings carry verbatim quoted passages + permalinks with `contested:` and syndication markers; explore answers cite `path:line` spans; research notepads end with a one-line `Answers the question: yes\|partial\|no` drift check; synthesis emits a Tensions section (claim vs claim, sources, adopted resolution + why). Prompt-layer only |
